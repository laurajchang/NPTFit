...
def return_xs_1break(double[::1] theta, double[::1] f_ary, double[::1] df_rho_div_f_ary, 
    ...
    for f_index in range(len(f_ary)):
        ...
        if ft_cond:
            ...
            # print("Calculated reference gamma functions")
            
            # sb_eff_ary = np.array([f * sb * f2 for f in ft_compressed]) 
            for p in range(npix_roi):
                for k in range(k_max):
                    if k <= data[p]+1:
                        sb_eff_mat[p,k] = ft_compressed[p] * sb 
            # print("Calculated sb_eff_mat")
            ...
            for k in range(k_max):
                ...
                g1_int_ary = interp.loginterp1d(sb_ref_ary, g1_ref_ary, sb_eff_ary)
                g1_mat_f[:,k] = g1_int_ary
                # print("g1_int_ary = ", g1_int_ary)

                minval = findmin.minval(g2_ref_ary)

                if minval <= 0:
                    g2_int_ary = interp.logloginterp1d(sb_ref_ary, g2_ref_ary, sb_eff_ary, offset = 1-minval) 
                else:
...
@cython.cdivision(True)
@cython.initializedcheck(False)
def return_xs_1break_orig(double[::1] theta, double[::1] f_ary, 
