...
def return_xs(double[::1] theta, double[::1] f_ary, double[::1] df_rho_div_f_ary, 
              double[::1] ftreg_mean_fluxes, int[::1] ftreg_map, 
    ...
    if n_break == 1:
        return return_xs_1break(theta, f_ary, df_rho_div_f_ary, ftreg_mean_fluxes, ftreg_map, npt_compressed,
                                data)
    elif n_break == 2:
        return return_xs_2break(theta, f_ary, df_rho_div_f_ary, ftreg_mean_fluxes, ftreg_map, npt_compressed,
                                data)
    else:
        print("Sorry, flux regions for ",n_break,"not yet implemented!")
    # elif n_break == 3:
    #     return return_xs_3break(theta, f_ary, df_rho_div_f_ary, ftreg_mean_fluxes, ftreg_map, npt_compressed,
    #                             data)
    # else:
...
def return_xs_1break(double[::1] theta, double[::1] f_ary, double[::1] df_rho_div_f_ary, 
    ...
    cdef int nftreg = len(ftreg_mean_fluxes)

    cdef double[:,::1] g1_mat_f = np.empty((nftreg, k_max+1), dtype=DTYPE)
    cdef double[:,::1] g2_mat_f = np.empty((nftreg, k_max+1), dtype=DTYPE)

    cdef double[::1] pref1_x_m_ary_ary = np.empty(nftreg, dtype=DTYPE) 
    cdef double[::1] pref2_x_m_ary_ary = np.empty(nftreg, dtype=DTYPE)   
    cdef double[::1] a_ps_ary = np.empty(nftreg, dtype=DTYPE)
    cdef double[::1] sb_ary = np.empty(nftreg, dtype=DTYPE)

    cdef bool ft_cond = False

    if len(ftreg_mean_fluxes) != 0:
        ft_cond = True
        # print("Nontrivial flux template provided, with mean fluxes ",np.asarray(ftreg_mean_fluxes))
    ...
    for f_index in range(len(f_ary)):
        ...
        if ft_cond:
            for i in range(nftreg):
                g1_ary_f = igf.incgamma_up_fct_ary(k_max, 1. - n1, ftreg_mean_fluxes[i] * sb * f2)
                g2_ary_f = igf.incgamma_lo_fct_ary(k_max, 1. - n2, ftreg_mean_fluxes[i] * sb * f2)
                g1_mat_f[i,:] = g1_ary_f
                g2_mat_f[i,:] = g2_ary_f

                pref1_x_m_ary = pow(ftreg_mean_fluxes[i] * sb * f2, n1)
                pref2_x_m_ary = pow(ftreg_mean_fluxes[i] * sb * f2, n2)  
                pref1_x_m_ary_ary[i] = pref1_x_m_ary
                pref2_x_m_ary_ary[i] = pref2_x_m_ary

                a_ps_ary[i] = a_ps/ftreg_mean_fluxes[i]
                sb_ary[i] = sb * ftreg_mean_fluxes[i]
            ...
            for p in range(npix_roi):
                ...
                for k in range(data[p]+1):
                    x_m_ary_f = a_ps_ary[ftreg_p] * (pref1_x_m_ary_ary[ftreg_p]*g1_mat_f[ftreg_p,k] \
                                + pref2_x_m_ary_ary[ftreg_p]*g2_mat_f[ftreg_p,k]) * npt_compressed[p]
...
def return_xs_2break(double[::1] theta, double[::1] f_ary, double[::1] df_rho_div_f_ary, 
    ...
    cdef double[:,::1] g0_mat_f = np.empty((nftreg, k_max+1), dtype=DTYPE)
    cdef double[:,::1] g1_mat_f = np.empty((nftreg, k_max+1), dtype=DTYPE)
    cdef double[:,::1] g2_mat_f = np.empty((nftreg, k_max+1), dtype=DTYPE)

    cdef double[::1] pref0_x_m_ary_ary = np.empty(nftreg, dtype=DTYPE)
    cdef double[::1] pref1_x_m_ary_ary = np.empty(nftreg, dtype=DTYPE) 
    cdef double[::1] pref2_x_m_ary_ary = np.empty(nftreg, dtype=DTYPE)   

    cdef double[::1] a_ps_ary = np.empty(nftreg, dtype=DTYPE)
    cdef double[::1] sb1_ary = np.empty(nftreg, dtype=DTYPE)
    cdef double[::1] sb2_ary = np.empty(nftreg, dtype=DTYPE)
    ...
    for f_index in range(len(f_ary)):
        ...
        if ft_cond:
            for i in range(nftreg):
                g0_ary_f = igf.incgamma_up_fct_ary(k_max, 1. - n1, ftreg_mean_fluxes[i] * sb1 * f2)
                g1_ary_f = igf.incgamma_up_fct_ary(k_max, 1. - n2, ftreg_mean_fluxes[i] * sb2 * f2) \
                           - igf.incgamma_up_fct_ary(k_max, 1. - n2, ftreg_mean_fluxes[i] * sb1 * f2)
                g2_ary_f = igf.incgamma_lo_fct_ary(k_max, 1. - n3, ftreg_mean_fluxes[i] * sb2 * f2)

                g0_mat_f[i,:] = g0_ary_f
                g1_mat_f[i,:] = g1_ary_f
                g2_mat_f[i,:] = g2_ary_f

                pref0_x_m_ary = pow(ftreg_mean_fluxes[i] * sb1 * f2, n1)
                pref1_x_m_ary = pref0_x_m_ary * pow(ftreg_mean_fluxes[i] * sb1 * f2, n2 - n1)
                pref2_x_m_ary = pref1_x_m_ary * pow(ftreg_mean_fluxes[i] * sb2 * f2, n3 - n2)

                pref0_x_m_ary_ary[i] = pref0_x_m_ary
                pref1_x_m_ary_ary[i] = pref1_x_m_ary
                pref2_x_m_ary_ary[i] = pref2_x_m_ary

                a_ps_ary[i] = a_ps/ftreg_mean_fluxes[i]
                sb1_ary[i] = sb1 * ftreg_mean_fluxes[i]
                sb2_ary[i] = sb2 * ftreg_mean_fluxes[i]
            ...
            for p in range(npix_roi):
                ...
                for k in range(data[p]+1):
                    x_m_ary_f = a_ps_ary[ftreg_p] * (pref0_x_m_ary_ary[ftreg_p] * g0_mat_f[ftreg_p,k] + pref1_x_m_ary_ary[ftreg_p]
                                * g1_mat_f[ftreg_p,k] + pref2_x_m_ary_ary[ftreg_p] * g2_mat_f[ftreg_p,k]) \
                                * npt_compressed[p]
                    x_m_ary[p,k] += df_rho_div_f2 * x_m_ary_f
